...
export default function CreateProjectTab({ step }: CreateProjectTabProps) {
  ...
  const completedClass: string = 'bg-green-500 text-green-100 border border-green-300';
  ...
  return (
    <section className="max-w-lg mx-auto bg-white grid grid-cols-2 gap-2 px-3 pt-4">
      <Link href={{ pathname: '/create-project', query: { step: STEP_DETAIL } }}>
        <a type="button" className={`transition-all p-1 w-full shadow rounded-full text-center font-bold text-sm ${isDetailCompleted ? completedClass : detailClass}`}>
          1. Informasi Proyek
        </a>
      </Link>
